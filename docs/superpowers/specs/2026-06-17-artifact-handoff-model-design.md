...
### Follow-ups after Plan B (surfaced during Plan B review — tracked, not yet done)

- **Legacy pipeline path drops declared outputs (decision needed).** Plan B wires the **steerable** path only (`/pipelines/steerable/start` → `SteerableAgentOrchestrator` → `RunAgentRoundActivity` → `FoundryAgentRuntime.RunRoundAsync`). The older **pipeline** path is still reachable (`/pipelines/start` → `TaskPipelineOrchestrator` → `InvokeAgentActivity` → `FoundryAgentRuntime.RunTurnAsync` → `AgentToolLoop`); there, the `declare_output`/`update_output`/`remove_output` tools are advertised (tools-v5 is global) but `AgentToolLoop` discards `RoundProcessResult.OutputOps` and `InvokeAgentActivity` never sets `Artifact.Outputs`. Net: an agent on that path can declare deliverables that silently vanish. **Decision:** either retire/disable `/pipelines/start` (the steerable rework is the strategic direction) or, if it stays, wire `OutputOps` through `AgentToolLoop.LoopResult` → `InvokeAgentActivity` artifact build (mirroring `RunAgentRoundActivity`). Tracked here rather than hidden in a code comment.
- **`update_output` content edits default content-type to Markdown** when `contentType` is omitted — overwriting a prior Json/Code type. Add a prompt note (agents should re-supply `contentType` when editing content) or carry the existing type forward in `OutputAccumulator`.
- **`PendingOutputs` patched as a full `Set` each round it changes** — RU cost grows with deliverable size × rounds. Fine for Document payloads; revisit when external/Code outputs (Spec 2) could carry larger data.
- **Integration glue in `RunAgentRoundActivity` (round-0 reset, accumulate→patch, `IsValid()` filter) is not unit-tested** (Durable/Cosmos-bound); the pure reducer is. Consider extracting a testable seam if this logic grows.
