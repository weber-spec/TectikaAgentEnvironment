...
public interface IWorkspaceService
{
    ...
    /// GitHub connection the entrypoint clones the repo (default branch); the agent creates its own
    /// working branch. <paramref name="canPush"/> reflects the agent's CanPushCode permission; when
    /// false the entrypoint disables the push remote so the workspace can read but not push.</summary>
    Task<WorkspaceInfo?> ProvisionAsync(Board board, string runId, bool canPush, CancellationToken ct = default);
