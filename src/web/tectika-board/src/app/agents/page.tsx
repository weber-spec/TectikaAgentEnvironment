...
import type { AgentRole, GitHubPermissions } from '@/lib/types';
...
function RoleEditor({ role, onSave, onClose }: { role: AgentRole; onSave: (r: AgentRole) => void | Promise<void>; onClose: () => void }) {
  ...
  const setGitHub = (patch: Partial<GitHubPermissions>) =>
    setR(prev => ({ ...prev, githubPermissions: { canRead: false, canCreateBranch: false, canPush: false, canCreatePr: false, ...prev.githubPermissions, ...patch } }));
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="rounded-lg border border-[var(--border)] p-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {([
              ['canRead', 'Read files & PRs'],
              ['canCreateBranch', 'Create branches'],
              ['canPush', 'Push files'],
              ['canCreatePr', 'Create pull requests'],
            ] as [keyof GitHubPermissions, string][]).map(([key, label]) => (
              <label key={key} className="flex items-center gap-2 text-sm text-[var(--foreground)]">
                <input type="checkbox"
                  checked={r.githubPermissions?.[key] ?? false}
                  onChange={e => setGitHub({ [key]: e.target.checked })}
